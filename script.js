 ...
 let allcloths=[];
  ...
  let value = parseInt(document.getElementsByClassName("new-product"))||0;
  ...
  const matchwordlady = allcloths.filter(item => item.username.toLowerCase().includes("ladys"));
  ...
  kidsclothsbtn.addEventListener("click",function(){
    const kidsmatchingcloths =allcloths.filter(item =>item.username.toLowerCase().includes("kid's"));
